...
def accuracy(typed, source):
    ...
    correct = 0
    for t, r in zip(typed_words, source_words):
        correct += (t == r)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff < smallest_diff:
            closest_word = word
            smallest_diff = diff
    return closest_word if smallest_diff <= limit else typed_word
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamps in timestamps_per_player:
        ...
        for i in range(1, len(timestamps)):
            player_times.append(timestamps[i] - timestamps[i - 1])  # 计算相邻时间戳的差值
...
def fastest_words(match):
    ...
    参数：
        match: 由 time_per_word 返回的匹配数据抽象。
    ...
    [4, 1, 6]
    """
    player_indices = range(len(get_all_times(match)))  # 包含每个玩家的索引
    word_indices = range(len(get_all_words(match)))    # 包含每个单词的索引
    ...
    for word_index in word_indices:
        fastest_player = 0  # 默认第一个玩家最快
        fastest_time = time(match, 0, word_index)  # 第一个玩家输入该单词的时间
