...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    def division(t,args_update):
        if args_update.rest is nil:
            t//=args_update.first
            return t
        else:
            if isinstance(args_update.first, Pair):
                args_update.first = calc_eval(args_update.first)
            t//=args_update.first
            return division(t,args_update.rest)
    return division(args.first,args.rest)
